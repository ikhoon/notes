...
class SetSpec extends WordSpec with Matchers {
  ...
  def intersect[A, B](as: List[A], bs: List[B]): List[(A, B)] = for {
    a <- as
    b <- bs if a == b
  } yield (a, b)
  ...
  "set" should {
    ...
    def diff[A, B](as: List[A], bs: List[B]): List[(Option[A], Option[B])] = for {
      a <- as
    ...
    "zip with union" in {
      ...
      val bs = List(2, 3, 4, 5)
      // intersect + (a - b) + (b - a)
      union(as, bs) shouldBe List((Some(2),Some(2)), (Some(3),Some(3)), (Some(1),None), (None,Some(4)), (None,Some(5)))
